...
int ltelc_api_default_pdp_context_read(pdp_context_info_t *populated_info)
{
	...
	//char *at_str_end = at_response_str;
	...
	/*while ((at_str_end = strstr(at_str_end, AT_CMD_RSP_DELIM)) != NULL)*/ {
		...
		ret = at_parser_max_params_from_str(
			at_response_str, NULL, &param_list,
			AT_CMD_PDP_CONTEXT_READ_PARAM_COUNT);
		if (ret) {
		...
		ret = at_params_string_get(
			...
			populated_info->pdp_type_str, &param_str_len);
		if (ret) {
			...
			if (strcmp(populated_info->pdp_type_str, "IPV4V6") ==
			    0) {
				populated_info->pdp_type = PDP_TYPE_IP4V6;
			} else if (strcmp(populated_info->pdp_type_str,
					  "IPV6") == 0) {
		...
		ret = at_params_string_get(
			...
			populated_info->ip_addr_str, &param_str_len);
		if (ret) {
			printf("Could not parse apn str, err: %d", ret);
			goto clean_exit;
		}
		...
		{
			char *tmp =
				malloc(strlen(populated_info->ip_addr_str) + 1);
...
void ltelc_api_modem_info_get_for_shell(const struct shell *shell)
{
	...
	if (ret >= 0) {
		...
		inet_ntop(AF_INET, &(pdp_context_info.sin4.sin_addr), ipv4_addr,
			  sizeof(ipv4_addr));
		inet_ntop(AF_INET6, &(pdp_context_info.sin6.sin6_addr),
		...
		shell_print(
			...
			"  IPv4 address:           %s\n"
			"  IPv6 address:           %s",
			pdp_context_info.cid, pdp_context_info.pdp_type_str,
