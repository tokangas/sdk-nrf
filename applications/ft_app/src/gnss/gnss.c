...
static void print_nmea(const struct gps_nmea *nmea)
{
	...
	shell_print(gnss_shell_global, "%s", print_buf);
}
		...
		struct gps_event *evt)
{
	...
	case GPS_EVT_SEARCH_TIMEOUT:
		if (event_output_level > 0) {
			shell_print(gnss_shell_global, "GNSS: Search timeout");
		}
		break;
	case GPS_EVT_PVT:
	...
	case GPS_EVT_OPERATION_BLOCKED:
		if (event_output_level > 0) {
			shell_print(gnss_shell_global, "GNSS: Operation blocked by LTE");
		}
		break;
	case GPS_EVT_OPERATION_UNBLOCKED:
		if (event_output_level > 0) {
			shell_print(gnss_shell_global, "GNSS: Operation unblocked");
		}
		break;
	case GPS_EVT_AGPS_DATA_NEEDED:
		...
		break;
	case GPS_EVT_ERROR:
		shell_error(gnss_shell_global, "GNSS: GPS error: %d",
			    evt->error);
		break;
	default:
		shell_error(gnss_shell_global, "GNSS: Unknown GPS event type: %d",
			    evt->type);
		break;
	}
}
...
static void gnss_init(void)
{
	...
	if (gps_dev == NULL) {
		shell_error(gnss_shell_global, "Could not get %s device",
