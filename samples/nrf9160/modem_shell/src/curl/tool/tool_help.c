...
static const struct helptxt helptext[] = {
  ...
  {"-d, --data #<count of generated bytes>",
   "MOSH specific option for HTTP POST data",
   CURLHELP_IMPORTANT | CURLHELP_HTTP | CURLHELP_POST | CURLHELP_UPLOAD},
  {"    --upload-buff-size #<size of the upload buffer>",
   "MOSH curl tool hook for the CURLOPT_UPLOAD_BUFFERSIZE curl lib option impacting upload sending",
   CURLHELP_IMPORTANT | CURLHELP_HTTP | CURLHELP_POST | CURLHELP_UPLOAD},
...
#else
  {"-o, --output /dev/null",
   "MOSH specific option to Write to /dev/null instead of stdout",
   CURLHELP_IMPORTANT | CURLHELP_CURL},
#endif
  {"-Z, --parallel",
