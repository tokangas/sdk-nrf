	...
	while (true) {
		if (ppp_modem_data_socket_fd < 0) {
			...
			if (ret > 0) { /* && (fds[0].revents & POLLIN) */
				recv_data_len =
					recv(ppp_modem_data_socket_fd,
					     receive_buffer,
					     PPP_MODEM_DATA_RCV_BUFFER_SIZE, 0);
				if (recv_data_len > 0) {
					if (sendto(ppp_data_socket_fd,
						   receive_buffer,
						   recv_data_len, 0,
						   (const struct sockaddr *)&dst,
						   sizeof(struct sockaddr_ll)) <
					    0) {
						printk("%s: cannot send data from mdm to PPP link - dropped data of len %d\n",
						       (__func__),
						       recv_data_len);
					}
				} else {
					printk("%s: recv() from modem failed %d\n",
					       (__func__), recv_data_len);
				}
			} else if (ret < 0) {
				printk("%s: poll() failed %d\n", (__func__),
				       ret);
			}
		}
	}
