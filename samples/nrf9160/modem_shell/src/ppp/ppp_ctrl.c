...
int ppp_modem_data_socket_fd; /* Socket to send and recv data to/from modem */
int ppp_data_socket_fd; /* Socket to send and recv data to/from Zephyr PPP link */
...
static void ppp_ctrl_net_mgmt_event_ip_levelhandler(struct net_mgmt_event_callback *cb,
			  uint32_t mgmt_event, struct net_if *iface)
{
	if ((mgmt_event & (NET_EVENT_IPV4_ADDR_ADD | NET_EVENT_IPV4_ADDR_DEL)) != mgmt_event) {
		return;
	}
	if (mgmt_event == NET_EVENT_IPV4_ADDR_ADD && iface == ppp_iface_global) {
		shell_print(ppp_shell_global, "Dial up connection up");
		return;
	}
	if (mgmt_event == NET_EVENT_IPV4_ADDR_DEL && iface == ppp_iface_global) {
		shell_print(ppp_shell_global, "Dial up connection down");
		return;
	}
...
static void ppp_ctrl_net_mgmt_events_subscribe()
{
	...
	net_mgmt_init_event_callback(&ip_level_net_mgmt_event_cb, ppp_ctrl_net_mgmt_event_ip_levelhandler,
...
int ppp_ctrl_start(const struct shell *shell) {
	...
	if (ppp_modem_data_socket_fd < 0) {
		...
		shell_info(shell, "modem data socket %d created for modem data", ppp_modem_data_socket_fd);
	}
	...
	//ppp_data_socket_fd = socket(AF_PACKET, SOCK_DGRAM, 666); //ETH_P_ALL);

	ppp_data_socket_fd = socket(AF_PACKET, SOCK_RAW, IPPROTO_RAW);
	if (ppp_data_socket_fd < 0) {
		shell_error(shell, "PPP data socket creation failed: (%d)!!!!\n", -errno);
		goto return_error;
	}
	else {
		shell_info(shell, "PPP data socket %d created", ppp_data_socket_fd);
	}
	...
	if (ret < 0) {
		shell_error(shell, "Failed to bind PPP data socket : %d", errno);
		goto return_error;
	}
