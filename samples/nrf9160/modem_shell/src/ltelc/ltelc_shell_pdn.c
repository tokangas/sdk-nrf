...
void ltelc_pdn_event_handler(uint8_t cid, enum pdn_event event, int reason)
{
	switch (event) {
	...
	if (family != NULL) {
		if (strcmp(family, "ipv4v6") == 0) {
			...
			*ret_fam = PDN_FAM_IPV6;
			   ...
			   strcmp(family, "non-ip") == 0) {
			*ret_fam = PDN_FAM_NONIP;
		} else {
			printk("ltelc_family_str_to_pdn_lib_family: could not decode PDN address family (%s)\n",
			       family);
			return -EINVAL;
		}
	} else {
...
int ltelc_shell_pdn_connect(const struct shell *shell, const char *apn_name, const char *family_str)
{
	...
	ret = pdn_ctx_create(&cid, ltelc_pdn_event_handler);
	if (ret) {
		shell_error(shell, "pdn_ctx_create() failed, err %d\n", ret);
		goto cleanup_and_fail;
	}

	ret = ltelc_family_str_to_pdn_lib_family(&pdn_lib_family, family_str);
	if (ret) {
		shell_error(
			shell,
			"ltelc_family_str_to_pdn_lib_family() failed, err %d\n",
			ret);
		goto cleanup_and_fail;
	}
...
void ltelc_shell_pdn_init(const struct shell *shell)
{
	...
	pdn_init();
	pdn_default_callback_set(ltelc_pdn_event_handler);
}