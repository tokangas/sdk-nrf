...
const char *
ltelc_shell_print_sleep_type_to_string(enum lte_lc_modem_sleep_type sleep_type, char *out_str_buff)
{
	...
	return ltelc_shell_map_to_string(mapping_table, sleep_type,
					 out_str_buff);
}

void ltelc_shell_print_modem_sleep_notif(const struct shell *shell,
					 const struct lte_lc_evt *const evt)
{
	...
	case LTE_LC_EVT_MODEM_SLEEP_EXIT_PRE_WARNING:
		...
		shell_print(
			...
			time_in_secs,
			ltelc_shell_print_sleep_type_to_string(modem_sleep.type,
							       snum));
		break;
	case LTE_LC_EVT_MODEM_SLEEP_EXIT:
		...
		shell_print(shell,
			    ...
			    time_in_secs,
			    ltelc_shell_print_sleep_type_to_string(
...
const char *ltelc_shell_sysmode_preferred_to_string(int sysmode_preference,
						    char *out_str_buff)
{
	struct mapping_tbl_item const mapping_table[] = {
	...
	return ltelc_shell_map_to_string(mapping_table, sysmode_preference,
					 out_str_buff);
}

const char *ltelc_shell_sysmode_currently_active_to_string(int actmode,
							   char *out_str_buff)
{
	struct mapping_tbl_item const mapping_table[] = {
...
const char *
ltelc_shell_map_to_string(struct mapping_tbl_item const *mapping_table,
